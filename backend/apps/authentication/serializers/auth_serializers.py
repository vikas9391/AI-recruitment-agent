...
class AdminCreateCompanySerializer(serializers.Serializer):
    """
    Used by a Super Admin to onboard a brand-new tenant: creates a
    Company plus its first HR_ADMIN user, atomically. Distinct from
    RegisterSerializer (self-service signup) in that the Super Admin
    sets the new admin's password directly rather than the new admin
    confirming it themselves, and no tokens are issued for the caller.
    """
